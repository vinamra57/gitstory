...
Adding a dependency to Tox is simple as well, but requires editing the tox file manually. You have to add a new line to the `deps` section of `tox.toml` from root, adding a line between the brackets consisting of (listed in `tox.toml` file):\
\
`"[package name]>=[min version number]",`\
\
You can also manually add dependencies to UV with this exact format in the `dependencies` section.
