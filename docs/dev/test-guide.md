# How to Test
Our tests use pytest, but everything regarding pytest will be automatically handled using our test handler tox.
## Running Tests
...
There are examples at `tests/test_sample.py` and `tests/test_sample_other.py` from root.