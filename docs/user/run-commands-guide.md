...
```
gitstory compare "<YOUR_REPO_LOCATION_PATH>" <base-branch> <compare-branch>
```
...
```
gitstory since "<YOUR_REPO_LOCATION_PATH>" <Nx -OR- YYYY-MM-DD>
```

We also have flags for certain commands:
| Command | Purpose | Runs with |
|---------|---------|---------|
| `--help` | Help command, provides description of specific command (--help) | ALL COMMANDS |
| `--since` | Summary *from* from time period (--start=4w) | `run`, `dashboard`, `compare` |
| `--until` | Summary *till* a certain time period (--until=2w) | `run`, `dashboard`, `since`, `compare` |
| `--branch` | Sets the branch the command needs to run on (--branch=feature) | `run`, `dashboard`, `since`|
