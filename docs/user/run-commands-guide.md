...
**Windows:**
`uv run python src/gitstory/__main__.py run "<YOUR_REPO_LOCATION_PATH>"`: equivalent to `gitstory run`, this runs GitStory and generates a code-summary in the CLI itself (i.e. its printed on the CLI to user)

`uv run python src/gitstory/__main__.py dashboard "<YOUR_REPO_LOCATION_PATH>"`: equivalent to `gitstory dashboard`, this generates a HTML file called `dashboard.html` in an `output` folder, located in at the root of your chosen Git repository (the output directory will be created by GitStory).
